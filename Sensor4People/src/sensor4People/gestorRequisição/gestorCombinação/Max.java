...
public class Max implements ICombinador {

	@Override
	/**
	 * (non-Javadoc)
	 * @see sensor4People.gestorRequisição.gestorCombinação.ICombinador#combinar(java.util.List)
	 * Exemplo de combinação onde o maior elemento é combinado
	 */
	public Informacao combinar(List<Informacao> infos) {
		...
		max = Integer.decode(info.getValor());
		while(it.hasNext()){
			info = it.next();
			if(Integer.decode(info.getValor()) > max){
				max = Integer.decode(info.getValor());
				i = infos.indexOf(info);
			}
		}
